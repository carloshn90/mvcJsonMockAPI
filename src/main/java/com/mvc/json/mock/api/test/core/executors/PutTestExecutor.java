...
public class PutTestExecutor implements TestExecutor {
    ...
    public Either<ErrorMessage, EndPointPayload> execute(MockMvc mockMvc, EndPointPayload endPointPayload) {
        ...
        try {
            TestExecutor.checkResponse(mockMvc.perform(createPut(request)), response);
        } catch (Throwable e) {
    ...
    private MockHttpServletRequestBuilder createPut(RequestPayload request) {

        MockHttpServletRequestBuilder req = put(request.getPath())
